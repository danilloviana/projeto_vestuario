...
public class GuardaRoupa {
    ...
    public static void main(String[] args) {
    	...
    	int numVest, numCalcas, numCam, op = 0, numeroCalca, numeroCamisa, vlng;
    	String corV, tecidoV, corCalca, tecidoCalca, corCamisa, tecidoCamisa, tipoCamisa;
    	boolean longo;
		...
		case 1:
			if (numVest > 0)
			{
				...
				vestidos[numVest-1] = new Vestido(corV, tecidoV, longo);
				numVest = numVest - 1;
			}
			else if (numVest <= 0)
			{
		...
		case 2:
			if (numCalcas > 0)
			{
				...
				numeroCalca = sc.nextInt();
				System.out.println("Insira cor: ");
				corCalca = sc.next();
				calcas[numCalcas-1] = new Calca(corCalca, tecidoCalca, numeroCalca);
		...
		case 3:
			if (numCam > 0)
			{
				...
				tecidoCamisa = sc.next();
				System.out.println("insira o tipo da Camisa");
				tipoCamisa = sc.next();
				System.out.println("Insira o número: ");
				numeroCamisa = sc.nextInt();
				camisas[numCam-1] = new Camisa(corCamisa, tecidoCamisa, tipoCamisa, numeroCamisa);
				numCam = numCam - 1;
			}
			else if (numCam <= 0)
			{
